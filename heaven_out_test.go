...
func Test_BuilderRevive(t *testing.T) {
	type TestGoner struct {
		gone.GonerFlag
		a int `gone:"config,a.b.c"`
	}

	err := gone.NewForTest(func(cemetery gone.Cemetery) error {
		cemetery.Bury(config.NewConfig())
		cemetery.ReplaceBury(config.NewConfigure())
		cemetery.ReplaceBury(logrus.NewLogger())

		return nil
	}).TestRun(&TestGoner{}, func(testGoner gone.Goner) {
		goner := testGoner.(*TestGoner)
		assert.Equal(t, goner.a, 100)
	})
