...
//
//	mockgen -package=gone_grpc -source=../../interface.go -mock_names=Server=CmuxServer
//
...
func (m *MockTomb) SetDefault(arg0 reflect.Type) gone.Tomb {
	m.ctrl.T.Helper()
...
func (m *MockCemetery) ReplaceBury(goner gone.Goner, options ...gone.GonerOption) error {
	m.ctrl.T.Helper()
	varargs := []any{goner}
	for _, a := range options {
		varargs = append(varargs, a)
...
func (m *MockHeaven) GetCemetery() gone.Cemetery {
	m.ctrl.T.Helper()
...
func (m *MockNamedGoner) GetGonerId() gone.GonerId {
	m.ctrl.T.Helper()
